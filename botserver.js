...
// const {MainBot} = require('./bot/welcome-users');

const { MainBot } = require('./dynamic-bot');
...
module.exports = function (WebApp, callback) {
    // Create HTTP server
    let server = restify.createServer();
    server.listen(process.env.port || process.env.PORT || 3978, function() {
        console.log(`\n${ server.name } listening to ${ server.url }`);
        console.log(`\nGet Bot Framework Emulator: https://aka.ms/botframework-emulator`);
        console.log(`\nTo talk to your bot, open azure-bot-framework.bot file in the Emulator`);
        WebApp(bot)
        callback()
    });
